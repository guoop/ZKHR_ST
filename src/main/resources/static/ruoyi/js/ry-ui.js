    ...
    $.extend({
        ...
        operate: {
            ...
            dispathAll: function() {
                ...
                $.modal.confirm("确认要下发选中的" + rows.length + "条数据吗?", function() {
                    var url = $.table._option.dispathUrl;
                    var data = { "ids": rows.join() };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            // 添加信息
            addWeight: function(id,weight) {
                $.modal.open("添加" + $.table._option.modalName, $.operate.addUrl(id),weight);
            },
            // 添加信息，以tab页展现
            addTab: function (id) {
